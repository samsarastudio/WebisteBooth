...
| `/admin` cannot delete leads (405 on `/api/leads`) | The running app is an old build. On the Pi: `git log -1 --oneline` should show the latest main, then `npm run build:standalone && pm2 restart frameflix`. Confirm with `curl -sI https://inmomentservices.com/api/leads` — `Allow` must include GET and DELETE, not only POST. |
